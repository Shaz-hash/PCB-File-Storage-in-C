    ...
    else
    {
        ...
        if (currentfolderIndex + 1 == count)
        {
            ...
            if (is_directory)
            {
                ...
                disk_write(3, &rootBlock);
                updateSize(folders, &rootBlock, currentfolderIndex, 0, 0, 4096);
            }
        }
        else
        {
...
int removeEntriesFromTable(int tableEntryNumber)
{
    ...
    for (int j = 0; j < DIRECTORY_ENTRIES_PER_BLOCK; j++)
    {
        ...
        if (strcmp(currentEntryBlock.directory_block.entries[j].name, "0") == 0)
        {
            continue;
        }
        else
